...
public class HeteroTrace extends com.opaleye.ganseq.GanseqTrace {
	private final double secondPeakCutoff = RootController.secondPeakCutoff;
	...
	public HeteroTrace(GanseqTrace trace) {
	...
	public Variant detectHeteroIndel() {
		...
		Vector<AlignedPoint> alignedPoints = RootController.alignedPoints;
		...
		int type = -1;
		if(direction==1) {
			coordiMap = Formatter.fwdCoordinateMap;
		}
		else {
			coordiMap = Formatter.revCoordinateMap;
		}
		try {
			...
			if (direction ==1)
				...
				variant = new Indel("hetero", direction, type, alignedIndelStartIndex, alignedIndelEndIndex, alignedDoublePeakStartIndex, indelSeq, true);
			else if (direction == -1)
	...
	public BufferedImage getHeteroImage(TreeSet<Integer> highlightRefSeq, TreeSet<Integer> highlightSubSeq) {
		...
		for(int i=0;i<sequenceLength;i++) {
			...
			//두번째 base 쓰기. Hetero indel detection 대상 구간 밖에서는 subtractedSeq2
				...
				g.drawString(Character.toString(refChar), Integer.max(0, xPos-3), traceHeight+23);
				g.setColor(Color.black);
			}
	 ...
	 */
	private double score (boolean[] secondPeakExist, int startIndex, int endIndex, int direction) {
		double ret = -1;
		double offset = 0.2;
