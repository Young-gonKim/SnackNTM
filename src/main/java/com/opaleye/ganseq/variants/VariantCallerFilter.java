package com.opaleye.ganseq.variants;

import java.util.Iterator;
import java.util.TreeSet;
import java.util.Vector;

import com.opaleye.ganseq.AlignedPoint;
import com.opaleye.ganseq.EquivExpression;
import com.opaleye.ganseq.Formatter;
import com.opaleye.ganseq.GanseqTrace;
import com.opaleye.ganseq.RootController;
import com.opaleye.ganseq.TwoPeaks;
...
public class VariantCallerFilter {
	...
	public VariantCallerFilter(boolean fwdLoaded, boolean revLoaded, int startRange, int endRange,
			Vector<Variant> heteroVariantList, GanseqTrace trimmedFwdTrace, GanseqTrace trimmedRevTrace) {
		super();
		this.fwdLoaded = fwdLoaded;
		this.revLoaded = revLoaded;
		this.startRange = startRange;
		this.endRange = endRange;
		this.heteroIndelList = heteroVariantList;
		this.trimmedFwdTrace = trimmedFwdTrace;
		this.trimmedRevTrace = trimmedRevTrace;
	}
		...
		if(fwdLoaded) {
			for(int i=0;i<trimmedFwdTrace.getSequenceLength();i++) {
				try {	//에러나면 continue (filtering 안하면 됨)
					...
					int index = Formatter.fwdCoordinateMap.get(new Integer(i+1));
		...
		if(revLoaded) {
			for(int i=0;i<trimmedRevTrace.getSequenceLength();i++) {
				try {	//에러나면 continue (filtering 안하면 됨)
					...
					int index = Formatter.revCoordinateMap.get(new Integer(i+1));
					AlignedPoint point = alignedPoints.get(index-1);
					if(base1 == point.getRefChar()) {
						refBase = base1;
						nextBase = base2;
			...
			if(fwdLoaded && ap.getFwdChar() != Formatter.gapChar && ap.getRefChar() != Formatter.gapChar) {	//Indel은 따로.
				...
				if(ap.getFwdChar()=='A' || ap.getFwdChar()=='T' || ap.getFwdChar()=='G' || ap.getFwdChar()=='C') {
					if(ap.getFwdChar() != ap.getRefChar()) {	
						variant = new SNV(ap.getRefChar(), ap.getFwdChar(), ap.getRevChar(), 1, ap.getStringCIndex(), (i+1), ap.getFwdTraceIndex(), ap.getRevTraceIndex(), ap.isCoding(), ap.getGIndex(), onTarget(i+1), "homo");
					...
					for(String baseString:IUPACList) {
						...
						if(ap.getRefChar() != baseChar) {
							variant = new SNV(ap.getRefChar(), baseChar, ap.getRevChar(), 1,  ap.getStringCIndex(), (i+1), ap.getFwdTraceIndex(), ap.getRevTraceIndex(), ap.isCoding(), ap.getGIndex(), onTarget(i+1), "hetero");
							variantList.add(variant);
						}
					}
				}
			...
			if(revLoaded && ap.getRevChar() != Formatter.gapChar && ap.getRefChar() != Formatter.gapChar) {	// Indel은 따로.
				...
				if(ap.getRevChar()=='A' || ap.getRevChar()=='T' || ap.getRevChar()=='G' || ap.getRevChar()=='C') {
					...
					for(String baseString:IUPACList) {
						char baseChar = baseString.charAt(0);
						if(ap.getRefChar() != baseChar) {
							variant = new SNV(ap.getRefChar(), ap.getFwdChar(), baseChar, -1, ap.getStringCIndex(), (i+1), ap.getFwdTraceIndex(), ap.getRevTraceIndex(), ap.isCoding(), ap.getGIndex(), onTarget(i+1), "hetero");
							if(variantList.contains(variant)) {	
								variantList.remove(variant);
								variant.setHitCount(2); 
							}
		...
		if(fwdLoaded) 
			for(int i=0;i<fwdHeteroIndelStartPoint;i++) {
				...
				if(ap.getRefChar() == Formatter.gapChar && ap.getFwdChar() != Formatter.gapChar) {
					...
					if(duplication) {
						variant = new Indel("homo", 1, Indel.duplication, dupStartIndex+1, dupStartIndex +insertedSeq.length(), dupStartIndex+1, insertedSeq, onTarget(dupStartIndex+1));
					}
					else
						variant = new Indel("homo", 1, Indel.insertion, i+1, j+1,i+1, insertedSeq, onTarget(i+1));
					variantList.add(variant);
		...
		if(fwdLoaded) 
			for(int i=0;i<fwdHeteroIndelStartPoint;i++) {
				...
				if(ap.getFwdChar() == Formatter.gapChar && ap.getRefChar() != Formatter.gapChar) {
					...
					variant = new Indel("homo", 1, Indel.deletion, i+1, j,i+1, "", onTarget(i+1));
					variantList.add(variant);
					//System.out.println(indel.getHGVS());
					i=j;
				}
	...
	private TreeSet<Variant> updateVariantList(TreeSet<Variant> originalVariantList, TreeSet<Variant> targetVariantList) throws Exception{
		...
		if(leftPos == shiftedRightPos) {	//deletion
			...
			Indel indel = new Indel("homo", 1, Indel.delins, getAlignedIndexFromGIndex(refIndexList.get(leftPos+1)), 
