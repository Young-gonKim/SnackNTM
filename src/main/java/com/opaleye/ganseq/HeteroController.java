...
public class HeteroController implements Initializable {
	...
	public void showImage() {
		...
		java.awt.image.BufferedImage awtImage = heteroTrace.getHeteroImage(highlightRefSeq, highlightSubSeq);
	...
	public void showResult() {
		...
		AlignedPoint ap1 = alignedPoints.get(indelStartIndex - 1);
		AlignedPoint ap2 = alignedPoints.get(indelEndIndex - 1);
		...
		firstSeqLabel.setFont(new Font("Consolas", 13));
		firstSeqLabel.setPrefSize(250, 10);

		secondSeqLabel.setFont(new Font("Consolas", 13));
	...
	private void adjustTracePane(int traceIndex) {
		...
		if(heteroTrace.getDirection() == 1) {
			newLength = Formatter.fwdNewLength;
			startOffset = Formatter.fwdStartOffset;
		}
		else {
			newLength = Formatter.revNewLength;
